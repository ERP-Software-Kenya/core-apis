...
@CommandHandlerStrict(UpdateProductPriceCommand)
export class UpdateProductPriceCommandHandler implements ICommandHandler<UpdateProductPriceCommand, Product> {
  constructor(
    ...
    @InjectPinoLogger(UpdateProductPriceCommandHandler.name) private readonly logger: PinoLogger,
  ) {}

  public async execute(command: UpdateProductPriceCommand): Promise<Product> {
    this.logger.info(`Executing ${UpdateProductPriceCommand.name} id=${command.id}`);
    const existing = await this.repo.getAsync(command.id);
    const patch = this.mapper.map(command, UpdateProductPriceCommand, Product);
    ...
    if (changedFields.length > 0) {
      ...
      this.activityLog.record({
        action:         EActivityAction.ProductPriceChanged,
        entityType:     'Product',
        entityId:       updated.id,
        organizationId: updated.organizationId,
